...
if __name__ == "__main__": 
	...
	for font_path in font_files:
		try:
			...
			# 获取字体的各种名称
			font_name = font_prop.get_name()           # 字体名称
			font_family = font_prop.get_family()[0]    # 字体族
			font_style = font_prop.get_style()         # 样式 (normal, italic, oblique)
			font_variant = font_prop.get_variant()     # 变体 (normal, small-caps)
			font_weight = font_prop.get_weight()       # 粗细 (normal, bold, etc.)
			font_stretch = font_prop.get_stretch()     # 拉伸
			...
			font_info = {
				...
				'html_css_name': font_name,  # HTML/CSS 中可用的名称
				'docx_name': font_name       # Word 文档中可用的名称
			}
			
			font_info_list.append(font_info)
			
		except Exception as e:
			print(f"读取字体失败 {font_path}: {e}")

	# 按字体名称排序
	font_info_list.sort(key=lambda x: x['name'].lower())

	with open(os.path.join(SRCPATH, "system_fonts.json"), 'w', encoding='utf-8') as f:
		json.dump(font_info_list, f, indent=4, ensure_ascii=False)

	print(f"\n已保存 {len(font_info_list)} 个字体信息到 system_fonts.json")